...
async function request(path,body,token=false,method=body?'POST':'GET'){const response=await fetch(path,{method,headers:{'content-type':'application/json',...(token&&getToken()?{authorization:`Bearer ${getToken()}`}:{})},body:body?JSON.stringify(body):undefined}),data=response.status===204?null:await response.json();if(!response.ok)throw Object.assign(Error(data?.error||'Verzoek mislukt.'),{status:response.status});return data;}
