...
export function Lesson({room,action,busy}){
  ...
  const openAttempt=()=>attempt.current??=api('quiz/start',{}).catch(e=>{attempt.current=null;throw e;});
  const submitQuiz=async()=>{try{const {attemptId}=await openAttempt();setResult(await api('quiz',{attemptId,answers}));}catch(e){if(e.status===410)setAnswers({});throw e;}finally{attempt.current=null;}};
  useEffect(()=>{let active=true;attempt.current=null;setPack(null);setError('');setAnswers({});setResult(null);api('day-pack').then(d=>{if(active)setPack(d);}).catch(e=>{if(active)setError(e.message);});return()=>{active=false;};},[room.day]);
  ...
  return <section className="panel content-panel"><p className="cyan">{lesson.kicker}</p><h2>{lesson.title}</h2><p className="lede">{lesson.lede}</p>{pack.steps?.length>0&&<><p className="cyan">{t('path.label')}</p><ProgressivePath steps={pack.steps}/></>}<div className="learning-loop">{lesson.loop.map((s,i)=><div key={s.label}><span>0{i+1}</span><strong>{s.label}</strong><small>{s.prompt}</small></div>)}</div><div className="worked"><BookOpen size={20}/><div><h3>{t('lesson.explained')}</h3><p>{lesson.workedExample}</p></div></div>{pack.materials?.length>0&&<><h3>{t('lesson.materials')}</h3><ul className="materials">{pack.materials.map(m=><li key={m.label}>{m.href?<a href={m.href} target={m.href.startsWith('http')?'_blank':undefined} rel="noreferrer">{m.label}</a>:<span>{m.label}: <strong>OPEN</strong> · {m.open}</span>}{m.note&&<small className="muted"> · {m.note}</small>}</li>)}</ul></>}{pack.labs?.length>0&&<section className="lab-slot" aria-label={t('lab.heading')}><h3>{t('lab.heading')}</h3>{pack.labs.map(lab=><LabEmbed key={lab.id} lab={lab} preview={room.me.role==='Facilitator'} saved={room.me.progressByDay?.[String(room.day)]?.labs?.[lab.id]}/>)}</section>}<h3>{t('lesson.quickCheck')}</h3><p className="muted">{t('lesson.quizHint')}</p><form onSubmit={e=>{e.preventDefault();action(submitQuiz);}}>{questions.map((q,i)=><fieldset key={q.id}><legend>{i+1}. {q.question}</legend>{q.options.map(o=><label className="radio" key={o.id}><input type="radio" name={q.id} required checked={answers[q.id]===o.id} onChange={()=>{setAnswers({...answers,[q.id]:o.id});openAttempt().catch(()=>{});}}/>{o.label}</label>)}</fieldset>)}<button className="gradient" disabled={busy||room.me.role==='Facilitator'}>{t('lesson.submit')}<ArrowRight size={17}/></button></form>{result&&<div className="notice" role="status"><strong>{result.score}/{result.total} · {routeName(t,result.route)}</strong><p>{result.note}</p><p>{t('lesson.soloHint')}</p></div>}</section>;
}
...
export function Solo({room,action,busy,onNavigate}){
  ...
  const trail=useFetched(participant?'tasks':null,[room.day,trailKey(room)]),tasks=trail?.tasks;
