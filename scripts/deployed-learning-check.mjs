...
await record('fresh_progress_and_debrief',async()=>{const route=await json('/game/day-route',{token:state.participants[0]});if(route.days.length!==7||route.days.some(d=>d.progress.evidenceCount!==1||!d.progress.hasReflection||!d.progress.hasHandoff||d.progress.route!=='guided'))throw Error('fresh route progress incomplete');const d=await json('/game/debrief',{token:state.host});if(d.day!==7||d.members.length!==4||!d.handoffs.length)throw Error('debrief incomplete');const exportResponse=await request('/game/debrief/export',{token:state.host});if(!exportResponse.res.ok||!String(exportResponse.data||'').includes('Squad-overdracht'))throw Error('debrief export missing');});
