...
let base;
try{base=new URL(rawUrl);if(base.protocol!=='https:'||base.username||base.password||base.pathname!=='/'||base.search||base.hash)throw Error('ACADEMY_URL must be an HTTPS origin without path, query, or fragment.');}catch(e){console.error(e.message);process.exit(1);}
if(!/^[0-9a-f]{40}$/i.test(expected)){console.error('EXPECTED_REVISION must be a full SHA.');process.exit(1);}
const checks=[],tokens=new Set(),state={};
const clean=v=>String(v??'error').replaceAll(hostKey,'[redacted]').replace(/Bearer\s+\S+/gi,'Bearer [redacted]');
const record=async(name,fn)=>{try{await fn();checks.push({name,status:'pass'});console.log(`PASS ${name}`);}catch(e){checks.push({name,status:'fail'});console.log(`FAIL ${name}: ${clean(e.message)}`);}};
async function request(route,{token,method='GET',body}={}){const res=await fetch(new URL(route,base),{method,headers:{...(body?{'content-type':'application/json'}:{}),...(token?{authorization:`Bearer ${token}`}:{})},body:body?JSON.stringify(body):undefined,signal:AbortSignal.timeout(30000)});const type=res.headers.get('content-type')||'';let data=null;try{data=type.includes('json')?await res.json():await res.text();}catch{}return {res,data};}
async function json(route,opts={}){const r=await request(route,opts);if(!r.res.ok)throw Error(`${opts.method||'GET'} ${route} HTTP ${r.res.status}`);return r.data;}
async function post(route,body,token){return json(route,{method:'POST',body,token});}
async function requireToken(result,label){if(!result?.token)throw Error(`${label} token missing`);tokens.add(result.token);return result.token;}
...
await record('fresh_progress_and_debrief',async()=>{const route=await json('/game/day-route',{token:state.participants[0]});if(route.days.length!==5||route.days.some(d=>d.progress.evidenceCount!==1||!d.progress.hasReflection||!d.progress.hasHandoff||d.progress.route!=='guided'))throw Error('fresh route progress incomplete');const d=await json('/game/debrief',{token:state.host});if(d.day!==5||d.members.length!==4||!d.handoffs.length)throw Error('debrief incomplete');const exportResponse=await request('/game/debrief/export',{token:state.host});if(!exportResponse.res.ok||!String(exportResponse.data||'').includes('Squad-overdracht'))throw Error('debrief export missing');});
await record('participant_debrief_forbidden',async()=>{const r=await request('/game/debrief',{token:state.participants[0]});if(r.res.status!==403)throw Error(`expected HTTP 403, got ${r.res.status}`);});
for(const file of ['n8n-repository-review.json','n8n-review-README.md','claude-code-review-README.md','day5-fictional-issue.md'])await record(`starter_${file.replaceAll(/[^a-z0-9]+/gi,'_')}`,async()=>{const r=await request(`/game/starter/${file}`,{token:state.participants[0]});if(r.res.status!==200||!String(r.data||'').length)throw Error(`expected HTTP 200 for ${file}`);});
