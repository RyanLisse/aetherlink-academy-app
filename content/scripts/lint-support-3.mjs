#!/usr/bin/env node
/**
 * Interim AC lint for content/courses/worldline-wave-2/support-3/
 ...
 * - starterPath must contain .ts and .go (day-03 code starters, not n8n JSON)
 */
...
const dayDir = path.join(root, "content/courses/worldline-wave-2/support-3");
...
for (const file of files) {
  ...
  if (!/^day:\s*support-3\s*$/m.test(fmMatch[1])) {
    fail(`${file}: day must be support-3`);
  }
  ...
  if (starter) {
    ...
    if (!existsSync(abs) || !statSync(abs).isDirectory()) {
      ...
      if (!names.some((n) => n.endsWith(".ts"))) {
        fail(`${file}: starterPath folder missing *.ts: ${rel}`);
      }
      if (!names.some((n) => n.endsWith(".go"))) {
        fail(`${file}: starterPath folder missing *.go: ${rel}`);
      }
    }
  }
}
...
console.log(`PASS: ${files.length} lessons OK under support-3`);
