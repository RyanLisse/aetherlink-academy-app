#!/usr/bin/env node
/**
 * Interim AC lint for content/courses/worldline-wave-2/teaching-2/
...
const dayDir = path.join(root, "content/courses/worldline-wave-2/teaching-2");
...
for (const file of files) {
  ...
  if (!/source:\s*\n/.test(fmMatch[1]) && !/^source:/m.test(fmMatch[1])) {
    fail(`${file}: missing source attribution frontmatter`);
  }
  if (!/aetherlink-classroom-slides/.test(fmMatch[1])) {
    fail(`${file}: source must cite aetherlink-classroom-slides`);
  }
  if (!/decision 3|decision.?3/i.test(fmMatch[1])) {
    fail(`${file}: source must cite CURRICULUM.md decision 3`);
  }
...
console.log(`PASS: ${files.length} lessons OK under teaching-2`);
