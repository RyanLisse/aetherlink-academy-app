#!/usr/bin/env node
/**
 * Interim AC lint for content/courses/worldline-wave-2/support-4/
 ...
 * - starterPath must contain analyze.mjs (day-04 fraud challenge starter)
 */
...
const dayDir = path.join(root, "content/courses/worldline-wave-2/support-4");
...
for (const file of files) {
  ...
  if (!/^day:\s*support-4\s*$/m.test(fmMatch[1])) {
    fail(`${file}: day must be support-4`);
  }
  ...
  if (starter) {
    ...
    if (!existsSync(abs) || !statSync(abs).isDirectory()) {
      ...
      if (!names.includes("analyze.mjs")) {
        fail(`${file}: starterPath folder missing analyze.mjs: ${rel}`);
      }
    }
  }
}
...
console.log(`PASS: ${files.length} lessons OK under support-4`);
