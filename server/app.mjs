...
export function createApp({dir,repository,presence,proofBase='http://127.0.0.1:4400',root=process.cwd(),hostKey,publicBaseUrl=process.env.ACADEMY_PUBLIC_URL||`http://127.0.0.1:${process.env.PORT||4317}`,googleClientId=process.env.GOOGLE_CLIENT_ID,googleClientSecret=process.env.GOOGLE_CLIENT_SECRET,facilitatorDomains=process.env.ACADEMY_FACILITATOR_DOMAINS,signingSecret=process.env.PROOF_COLLAB_SIGNING_SECRET,fetchImpl=fetch,slidesService,labOrigins=process.env.ACADEMY_LAB_ORIGINS,labsForDay=day=>getDayPack(day)?.labs,labKeys=labGradingKeys,trustProxy=process.env.ACADEMY_TRUST_PROXY,chatConfig=readChatConfig(),mailer=createMailTransport()}={}){
 ...
 const chosenDay=(context,raw)=>{if(raw===undefined||raw===null||raw==='')return context.r.day;const day=Number(raw);if(!Number.isInteger(day)||!getDayPack(day))fail(404,`Geen contentpakket voor dag ${String(raw).slice(0,8)}.`);if(!courseDays(context.r).includes(day))fail(404,`Dag ${day} zit niet in de cursus.`);if(!readableDays(context).includes(day))fail(403,`Dag ${day} is nog niet vrijgegeven.`);return day;};
 ...
 app.get('/game/knowledge',wrap(async(req,res)=>{const pack=getDayPack(chosenDay(await browser(req),req.query.day));res.json({lessons:searchKnowledge(String(req.query.q||'')),mission:pack?.mission||mission});}));
 ...
 app.post('/game/chat',wrap(async(req,res)=>{const {r,s}=await browser(req);if(s.personId!=='facilitator'&&r.chat===false)fail(403,'De facilitator heeft de chat voor deze kamer uitgezet.');res.json(answerQuestion({day:r.day,released:readableDays({r,s}),query:text(req.body?.q,300),locale:req.body?.locale}));}));
 ...
 app.get('/game/chat/embed',wrap(async(req,res)=>{
  ...
  if(!p||s.readOnly){res.status(403).type('text/html').send(chatEmbedErrorHtml());return;}
  try{res.redirect(302,await createChatEmbedStartUrl({roomId:r.id,participantId:p.id},{config:chatConfig,fetchImpl}));}
  catch(e){console.warn('[academy] chat embed failed',{code:e.code||null});res.status(502).type('text/html').send(chatEmbedErrorHtml());}
 }));
