...
export function createApp({dir,repository,presence,proofBase='http://127.0.0.1:4400',root=process.cwd(),hostKey,publicBaseUrl=process.env.ACADEMY_PUBLIC_URL||`http://127.0.0.1:${process.env.PORT||4317}`,googleClientId=process.env.GOOGLE_CLIENT_ID,googleClientSecret=process.env.GOOGLE_CLIENT_SECRET,facilitatorDomains=process.env.ACADEMY_FACILITATOR_DOMAINS,signingSecret=process.env.PROOF_COLLAB_SIGNING_SECRET,fetchImpl=fetch,slidesService,labOrigins=process.env.ACADEMY_LAB_ORIGINS,labsForDay=day=>getDayPack(day)?.labs,labKeys=labGradingKeys,trustProxy=process.env.ACADEMY_TRUST_PROXY}={}){
 ...
 const chosenDay=(context,raw)=>{if(raw===undefined||raw===null||raw==='')return context.r.day;const day=Number(raw);if(!Number.isInteger(day)||!getDayPack(day))fail(404,`Geen contentpakket voor dag ${String(raw).slice(0,8)}.`);if(!readableDays(context).includes(day))fail(403,`Dag ${day} is nog niet vrijgegeven.`);return day;};
 ...
 app.get('/game/day-pack',wrap(async(req,res)=>{const context=await browser(req),day=chosenDay(context,req.query.day),pack=getDayPack(day);if(!pack)fail(400,`Geen contentpakket voor supportdag ${day}.`);res.json(publicDayPack(pack));}));
 ...
 app.get('/game/day-route',wrap(async(req,res)=>{const context=await browser(req),{r,p}=context,released=readableDays(context);res.json({day:r.day,released,days:listRouteDays().map(d=>({...d,released:released.includes(d.day),labsTotal:dayLabs(d.day).length,progress:dayProgress(r,p,d.day)}))});}));
