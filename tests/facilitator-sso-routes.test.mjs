...
import {readLoginState} from '../server/google-sso.mjs';
...
test('Google facilitator login authorizes routes without weakening the host-key boundary',async()=>{
 ...
 const expiredState='expired-server-state';await instance.store.loginStateSave({stateHash:createHash('sha256').update(expiredState).digest('hex'),nonce:'expired-nonce',codeVerifier:'expired-verifier',expiresAt:Date.now()-1});const expired=await invoke(instance.app,'/auth/google/callback',{query:{code:'authorization-code',state:expiredState},cookies:{}});assert.equal(expired.statusCode,302);assert.equal(expired.headers.location,'/?login_error=state');
 const cookieStart=await invoke(instance.app,'/auth/google/start');const cookieLogin=cookie(cookieStart,'academy-login'),cookieState=readLoginState(decodeURIComponent(cookieLogin),loginSecret);nonce=cookieState.nonce;const callback=await invoke(instance.app,'/auth/google/callback',{query:{code:'authorization-code',state:cookieState.state},cookies:{'academy-login':cookieLogin}});assert.equal(callback.statusCode,302);assert.equal(callback.headers.location,'/?facilitator=1');const facilitatorCookie=cookie(callback,'academy-facilitator');assert.ok(facilitatorCookie);
 const cookieReplay=await invoke(instance.app,'/auth/google/callback',{query:{code:'authorization-code',state:cookieState.state},cookies:{}});assert.equal(cookieReplay.statusCode,302);assert.equal(cookieReplay.headers.location,'/?login_error=state');
 const denied=await invoke(instance.app,'/game/create',{body:{name:'Denied squad'}});assert.equal(denied.statusCode,403);assert.deepEqual(denied.body,{error:'Ongeldige facilitator-startsleutel.'});
 const me=await invoke(instance.app,'/game/facilitator/me',{cookies:{'academy-facilitator':facilitatorCookie}});assert.equal(me.statusCode,200);assert.deepEqual(me.body,{email:'route@allowed.example',name:'Route Facilitator'});assert.equal((await invoke(instance.app,'/game/facilitator/me')).statusCode,401);
 const created=await invoke(instance.app,'/game/create',{body:{name:'SSO squad'},cookies:{'academy-facilitator':facilitatorCookie}});assert.equal(created.statusCode,200);assert.ok(created.body.token);const overview=await invoke(instance.app,'/game/facilitator/overview',{cookies:{'academy-facilitator':facilitatorCookie}});assert.equal(overview.statusCode,200);assert.deepEqual(overview.body[0].createdBy,{email:'route@allowed.example',name:'Route Facilitator'});const attached=await invoke(instance.app,'/game/facilitator/attach',{body:{roomId:created.body.roomId},cookies:{'academy-facilitator':facilitatorCookie}});assert.equal(instance.store.view(instance.store.auth(attached.body.token).r,instance.store.auth(attached.body.token).s).me.name,'Route Facilitator');
 const logout=await invoke(instance.app,'/auth/logout',{cookies:{'academy-facilitator':facilitatorCookie}});assert.equal(logout.statusCode,204);assert.match(logout.cookies.join('\n'),/academy-facilitator=;/);assert.equal((await invoke(instance.app,'/game/facilitator/me',{cookies:{'academy-facilitator':facilitatorCookie}})).statusCode,401);
});


test('facilitator session store failure redirects to login_error=session not verify',async()=>{
 const {publicKey,privateKey}=generateKeyPairSync('rsa',{modulusLength:2048}),jwk={...publicKey.export({format:'jwk'}),kid:'session-key',alg:'RS256',use:'sig'};let nonce;
 const encode=value=>Buffer.from(JSON.stringify(value)).toString('base64url'),issue=()=>{const header=encode({alg:'RS256',kid:jwk.kid}),now=Math.floor(Date.now()/1000),payload=encode({iss:'https://accounts.google.com',aud:'session-client',sub:'session-subject',email:'session@allowed.example',email_verified:true,name:'Session Facilitator',hd:'allowed.example',nonce,iat:now,exp:now+600}),signature=sign('RSA-SHA256',Buffer.from(`${header}.${payload}`),privateKey).toString('base64url');return `${header}.${payload}.${signature}`;};
 const fetchImpl=async url=>{if(url==='https://accounts.google.com/.well-known/openid-configuration')return Response.json({issuer:'https://accounts.google.com',authorization_endpoint:'https://accounts.google.com/o/oauth2/v2/auth',token_endpoint:'https://oauth2.googleapis.com/token',jwks_uri:'https://www.googleapis.com/oauth2/v3/certs'});if(url==='https://www.googleapis.com/oauth2/v3/certs')return Response.json({keys:[jwk]});if(url==='https://oauth2.googleapis.com/token')return Response.json({id_token:issue()});throw Error(`Unexpected URL ${url}`);};
 const signingSecret='session-signing-secret',loginSecret=createHmac('sha256',signingSecret).update('academy-login-state').digest(),instance=createApp({dir:mkdtempSync(path.join(os.tmpdir(),'academy-sso-session-')),hostKey:'break-glass-key',signingSecret,publicBaseUrl:'https://academy.example.test',googleClientId:'session-client',googleClientSecret:'session-secret',facilitatorDomains:'allowed.example',fetchImpl});
 const original=instance.store.facilitatorLogin.bind(instance.store);instance.store.facilitatorLogin=async()=>{throw Object.assign(new Error('insert into facilitator_sessions failed'),{code:undefined});};
 ...
 const warnings=[];const warn=console.warn;console.warn=(...args)=>{warnings.push(args);};try{
  const failed=await invoke(instance.app,'/auth/google/callback',{query:{code:'authorization-code',state:loginState.state},cookies:{'academy-login':loginCookie}});
