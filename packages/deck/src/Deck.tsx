...
function DeckContent({slides, index, revealStep, mode, onIndexChange, onRevealStepChange, presence}: DeckProps) {
  ...
  return <div className={`academy-deck mode-${mode} deck-type-${type}${activeSlide.dark ? " spotlight" : ""}${visual(activeSlide).keynote === true ? " keynote" : ""}`} data-type={type} data-face={visual(activeSlide).keynote === true ? "keynote" : undefined} data-opener={typeof visual(activeSlide).opener === 'string' ? visual(activeSlide).opener : ''} data-index={index} data-total={total}>
